...
const PromotionAdd: React.FC = () => {
  const [{ error: errorMessage, loading: PromotionLoading }, executePromotion] = useAxios({ url: '/api/promotion', method: 'POST' }, { manual: true });
  ...
  const handleSubmit = async (event: React.MouseEvent<HTMLElement>) => {
    ...
    if (missingFields.length > 0) {
      ...
      try {
        ...
        const response = await executePromotion({ data });
